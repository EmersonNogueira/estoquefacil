...
class SolicitacaoController extends Controller {
    ...
    private function checkAccess() {
        ...
        if (isset($_SESSION['tipo'])) {
            ...
            if ($tipoUsuario === 'admin' || $tipoUsuario === 'infra') {
                return;
            }
    
            // Permite acesso para solicitante apenas à rota específica
            if ($tipoUsuario === 'solicitante' && strpos($_SERVER['REQUEST_URI'], $this->base_url . 'Solicitacao/solicitacaoproduto') === 0) {
                return;
            }
        }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($_POST['id_produto']) && isset($_POST['quantidade']) && isset($_POST['nome'])) {
                ...
                if (isset($_SESSION['solicitacao'][$idProduto])) {
                    ...
                    if($temp>$saldo){
                        echo json_encode(['status' => 'error', 'message' => 'Quantidade inválida.saldo'.$saldo]);
                        return;
                    }else{
